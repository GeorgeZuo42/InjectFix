            ...
            try
            {
                ...
                try
                {
                    ...
                    assembly = AssemblyDefinition.ReadAssembly(assmeblyPath,
                        new ReaderParameters { ReadSymbols = true, InMemory = true });
                }
                catch
                {
                    //如果读取不到符号则不读
                    UnityEngine.Debug.LogWarning("read " + assmeblyPath + " with symbol fail");
                    //写入的时候用这个标志
                    readSymbols = false;
                    assembly = AssemblyDefinition.ReadAssembly(assmeblyPath,
                ...
                ilfixAassembly = AssemblyDefinition.ReadAssembly(args[1], new ReaderParameters { ReadSymbols = true, InMemory = true });
                ...
                else
                {
                        ...
                        == CodeTranslator.ProcessResult.Processed)
                    {
                        ...
                        UnityEngine.Debug.LogError("the new assembly must not be inject, please reimport the project!");
                        return;
                    }
