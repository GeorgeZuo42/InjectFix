...
namespace IFix.Editor
{
    ...
    public class IFixEditor
    {
        ...
        public static bool InjectAssembly(KeyValuePair<string,string> assembly)
        {
                ...
                FileAccess.Write)))
            {
                ...
                foreach (var kv in configure)
                {
                    ...
                    var typeList = kv.Value.Where(item => item.Key is Type)
                        ...
                        .Where(item => item.Key.Assembly.GetName().Name == assembly.Key)
                        .ToList();
            ...
            if (hasSomethingToDo)
            {
                ...
                var core_path = $"{GetScriptAssembliesFolder()}/IFix.Core.dll";
                var assembly_path = string.Format("{0}/{1}.dll", assembly.Value, assembly.Key);
        ...
        public static void InjectAllAssemblys()
        {
            ...
            bool hasThingToDo = false;
            foreach (var assembly in injectAssemblys)
            {
                hasThingToDo = InjectAssembly(assembly);
            }
            if (!hasThingToDo)
            {
                UnityEngine.Debug.Log("Nothing to Inject!");
            }
        ...
        static Dictionary<string, string> injectAssemblys = new Dictionary<string, string>
        {
            { "Assembly-CSharp",GetScriptAssembliesFolder()},
            { "Assembly-CSharp-firstpass",GetScriptAssembliesFolder() },
            {"MoonClient","./Assets/Plugins/GameLibs" }
        };
        ...
        static void doBackup(string ts)
        {
            ...
            foreach (var assembly in injectAssemblys)
            {
                var dllFile = string.Format("{0}/{1}.dll", assembly.Value, assembly.Key);
                if (!File.Exists(dllFile))
                {
                    continue;
                }
                File.Copy(dllFile, string.Format("{0}/{1}-{2}.dll", BACKUP_PATH, assembly.Key, ts), true);

                var mdbFile = string.Format("{0}{1}.dll.mdb", assembly.Value, assembly.Key);
                if (File.Exists(mdbFile))
                {
                    File.Copy(mdbFile, string.Format("{0}/{1}-{2}.dll.mdb", BACKUP_PATH, assembly.Key, ts), true);
                }

                var pdbFile = string.Format("{0}{1}.dll.pdb", assembly.Value, assembly.Key);
                if (File.Exists(pdbFile))
                {
                    File.Copy(pdbFile, string.Format("{0}/{1}-{2}.dll.pdb", BACKUP_PATH, assembly.Key, ts), true);
                }
            }
        }
        ...
        static void doRestore(string ts)
        {
            ...
            foreach (var assembly in injectAssemblys)
            {
                var dllFile = string.Format("{0}/{1}-{2}.dll", BACKUP_PATH, assembly.Key, ts);
                if (!File.Exists(dllFile))
                {
                    continue;
                }
                File.Copy(dllFile, string.Format("{0}{1}.dll", assembly.Value, assembly.Key), true);
                UnityEngine.Debug.Log("Revert to: " + dllFile);

                var mdbFile = string.Format("{0}/{1}-{2}.dll.mdb", BACKUP_PATH, assembly.Key, ts);
                if (File.Exists(mdbFile))
                {
                    File.Copy(mdbFile, string.Format("{0}{1}.dll.mdb", assembly.Value, assembly.Key), true);
                    UnityEngine.Debug.Log("Revert to: " + mdbFile);
                }

                var pdbFile = string.Format("{0}/{1}-{2}.dll.pdb", BACKUP_PATH, assembly.Key, ts);
                if (File.Exists(pdbFile))
                {
                    File.Copy(pdbFile, string.Format("{0}{1}.dll.pdb", assembly.Value, assembly.Key), true);
            ...
            string corePath = "./Assets/Plugins/IFix.Core.dll")
        {
            ...
            foreach (var assembly in injectAssemblys)
            {
                GenPatch(assembly.Key, string.Format("{0}/{1}.dll", assembly.Value, assembly.Key),
                    $"{GetScriptAssembliesFolder()}/IFix.Core.dll", string.Format("{0}/{1}.patch.bytes", patchOutputDir, assembly.Key));
            }
#else
        ...
        public static void GenPatch(string assembly, string assemblyCSharpPath, string corePath, string patchPath)
        {
        ...
        public static void Patch()
        {
            ...
            try
            {
                ...
                {
                    var assembly_path = string.Format("{0}/{1}.dll", assembly.Value, assembly.Key);
                    GenPatch(assembly.Key, assembly_path, $"{GetScriptAssembliesFolder()}/IFix.Core.dll",
                        string.Format("{0}.patch.bytes", assembly.Key));
                }
            }
            catch (Exception e)
            {
